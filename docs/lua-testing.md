...
Kafka, and it does not require a Sink configuration file. Pass `--config` and `--store-config` when
production overrides `execution.merge.lua` limits; the command validates the file and
...
| `--config` | No | Role YAML configuration, paired with `--store-config`, whose `execution.merge.lua` limits are applied. No backend is opened. |
