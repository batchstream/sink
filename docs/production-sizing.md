...
Store `max_concurrent` (default 64 for MongoDB and 128 for Elasticsearch/OpenSearch). The limit applies
per Engine/Worker process. Batch queues, Gateway fanout/connections, backend pools, Kafka
...
instances start conservatively and independently adapt. Do not raise a Store's
per-process concurrency ceiling in response to HPA, Kafka lag or queue growth.
