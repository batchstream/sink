...
func TestEngineDistinguishesAdmissionFromUnknownNativeOutcome(t *testing.T) {
	for _, failure := range []error{backpressure.ErrQueueFull, status.Error(codes.ResourceExhausted, "backend outcome unknown")} {
		...
		notStarted := failure == backpressure.ErrQueueFull
