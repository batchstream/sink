// Adapted from github.com/iceisfun/golua/stdlib/pattern.go at v1.1.1.
// Copyright 2026 github.com/iceisfun. See lua_pattern.LICENSE.
// Sink adds VM checkpoints to native matching, parsing and backtracking loops.
// Compare this copy with upstream when upgrading golua; parity tests exercise
// both matchers to keep the accepted syntax and capture results aligned.
...
// Charge one VM checkpoint per 256 units of native work, and at entry.
// The counter includes parsing and backtracking, even when nothing matches.
type luaPatternBudget struct {
	state *vm.VM
	steps uint64
}

func (b *luaPatternBudget) step() {
	if b.steps%256 == 0 {
...
func (ms *matchState) match(si int, pp int) int {
	...
	for {
		...
		elem, elemLen := ms.getPatternElem(ms.p, pp)
		if elem == nil {
			return -1
		}
...
func (ms *matchState) matchFrontier(si, pp int) (int, int, bool) {
	...
	set, setLen := ms.parseCharSetAt(ms.p, pp+2)
	if set == nil {
		return 0, 0, false
	}
	var prev byte = 0
...
func (ms *matchState) matchBackRef(si, pp int) (int, bool) {
	...
	for offset := 0; offset < len(capStr); offset++ {
		ms.budget.step()
		if ms.s[si+offset] != capStr[offset] {
			return 0, false
		}
	}
...
func luaMatchAt(budget *luaPatternBudget, s, pattern string, pos int) (int, []captureValue, bool) {
	ms := &matchState{budget: budget, s: s, p: pattern}
	end := ms.match(pos, 0)
	if end == -1 {
		return 0, nil, false
	}
...
func luaMatchFrom(state *vm.VM, s, pattern string, init int) (int, int, []captureValue, bool) {
	if init < 0 {
		init = len(s) + init + 1
	}
	if init < 1 {
	...
	if anchored {
		end, caps, ok := luaMatchAt(budget, s, searchPat, start0)
		if ok {
	...
	for i := start0; i <= len(s); i++ {
		end, caps, ok := luaMatchAt(budget, s, searchPat, i)
		if ok {
...
func (ms *matchState) getPatternElem(pattern string, patPos int) (patternElem, int) {
	...
	if ch == '%' {
		...
		case 'a', 'd', 'g', 's', 'w', 'l', 'u', 'c', 'p', 'x', 'z':
			element := classChar{class: next, neg: false}
			return element, 2
		case 'A', 'D', 'G', 'S', 'W', 'L', 'U', 'C', 'P', 'X', 'Z':
			element := classChar{class: next + 32, neg: true}
			return element, 2
		default:
			element := literalChar{ch: next}
			return element, 2
		}
	}

	if ch == '[' {
		set, setLen := ms.parseCharSetAt(pattern, patPos)
		if set == nil {
			return nil, 0
		}
		return set, setLen
	...
	element := literalChar{ch: ch}
	return element, 1
}

func (ms *matchState) parseCharSetAt(pattern string, patPos int) (*charSet, int) {
	neg := false
	start := patPos + 1
	...
	elems := ms.parseCharSetElems(pattern[start:end])
	result := &charSet{budget: ms.budget, elems: elems, neg: neg}
	return result, end - patPos + 1
}

func (ms *matchState) parseCharSetElems(set string) []patternElem {
	...
	for i < len(set) {
		...
		if set[i] == '%' && i+1 < len(set) {
			...
			element := rangeChar{low: set[i], high: set[i+2]}
			elems = append(elems, element)
			i += 3
		} else {
			element := literalChar{ch: set[i]}
			elems = append(elems, element)
			i++
		}
	}
