...
scripts/render-release-notes.sh \
  "${current_body}" \
  "${first_output}" \
  'ghcr.io/batchstream/sink:1.2.3' \
  'sha256:abc123'
scripts/render-release-notes.sh \
  "${first_output}" \
  "${second_output}" \
...
grep -Fq '`ghcr.io/batchstream/sink:1.2.3`' "${second_output}"
grep -Fq 'docker pull ghcr.io/batchstream/sink:1.2.3' "${second_output}"
grep -Fq '`ghcr.io/batchstream/sink@sha256:abc123`' "${second_output}"
